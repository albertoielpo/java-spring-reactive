...
import net.ielpo.reactivestack.config.ReactiveRedisComponent;
...
public class CacheController {
    ...
    public Mono<TestDto> save(@PathVariable String id) {
        ...
        return reactiveRedisComponent.set(cacheName, test.getA(), test).map(x -> test);
    }

    @GetMapping(path = "/{id}", consumes = MediaType.APPLICATION_JSON_VALUE, produces = MediaType.APPLICATION_JSON_VALUE)
    public Mono<BaseRes<TestDto>> get(@PathVariable String id) {
        try {
            Mono<TestDto> accessLogs = reactiveRedisComponent.get(cacheName, id).flatMap(x -> {
